...
export function Hero({ movie, isLoading }) {
    ...
    if (isLoading) return <SkeletonHeroMovie />;
    else
        return (
...
function SkeletonHeroMovie() {
    return (
        <div className="relative h-[77vh] w-full  bg-gray-600 flex items-center overflow-hidden animate-pulse">
            {/* Content */}
            <div className="absolute z-10 px-12 max-w-2xl">
                <h1 className="text-5xl md:text-7xl font-bold bg-gray-700 h-16 w-64 mb-4 leading-tight uppercase"></h1>

                <div className="flex items-center space-x-4 mb-8 text-sm font-medium">
                    <span className="bg-gray-300 h-4 w-16 "></span>
                    <span className="bg-gray-500 font-bold  h-4 w-16 "></span>
                    <span className="text-gray-300"></span>
                </div>

                <div className="flex items-center space-x-4">
                    <button className="flex items-center justify-center bg-gray-400 h-12 w-32 text-white font-bold py-3 px-10 rounded-full transition-colors"></button>

                    <button className="flex items-center justify-center border-2 border-gray-400 rounded-full w-12 h-12 bg-gray-300"></button>
                </div>
            </div>
        </div>
    );
